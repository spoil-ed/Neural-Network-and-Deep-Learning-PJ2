...
\centering
\small
\caption{Part~1 scoring-oriented summary. Runtime is the measured average
per-epoch time where it was logged; all runs use CIFAR--10 training from
scratch.}
\label{tab:part1score}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.19\textwidth}|c|c|c|>{\raggedright\arraybackslash}X}
\hline
Model / setting & Params & Time / epoch & Best acc (\%) & Role in the report \\
\hline
Raw-tensor ConvNet & 1.42M & \manualNetEpochTime{}s & \manualNetAcc{} &
manual convolution, pooling, linear layers, activation, and loss \\
Configurable ConvNet & 5.56M--20.1M & -- & 90.91--\convAcc{} &
required components and controlled ablation platform \\
CIFAR ResNet--18 & \baseParams{} & \bestEpochTime{}s & \baseAcc{} &
CIFAR stem, residual baseline, no ImageNet pretraining \\
ResNet--18 + manual SGD & \baseParams{} & \manualEpochTime{}s & \manualAcc{} &
hand-written SGD with Nesterov momentum and weight decay \\
WRN-28-10 + strong recipe & \bestParams{} & $\approx$\wrnEpochTime{}s &
\bestAcc{} & accuracy ceiling with AutoAugment, Cutout, Mixup/CutMix, EMA,
and TTA \\
\hline
\end{tabularx}
\end{table}
...
\label{tab:optional}
\begin{tabular}{c|p{0.31\textwidth}|p{0.29\textwidth}|c}
\hline
Block & Configuration & Main setting & Best acc (\%) \\
\hline
\multirow{9}{*}{Architecture}
& Baseline BN ConvNet & 5.56M params & 91.27 \\
& No normalization & 5.56M params & 90.05 \\
& GroupNorm & 5.56M params & 89.87 \\
& Average pooling & 5.56M params & 90.89 \\
& Deeper stages & 8.69M params & \textbf{93.08} \\
& Narrow classifier & 5.03M params & 91.07 \\
& Wide classifier & 6.61M params & 91.08 \\
& Conv dropout & 5.56M params & 90.98 \\
& BN + residual + dropout & 5.73M params & 91.50 \\
\hline
\multirow{8}{*}{Training}
& Standard crop+flip & baseline augmentation + cosine LR & 91.27 \\
& No augmentation & normalization only & 86.46 \\
& Cutout & crop+flip + random erasing & 91.44 \\
& No weight decay & $L_2$ disabled & 89.66 \\
& Label smoothing & smoothing $0.1$ & \textbf{91.74} \\
& Step LR & multistep decay & 90.86 \\
& Constant LR & no decay & 85.60 \\
& Combined regularization & cutout + dropout + smoothing & 91.60 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Accuracy-push sweep. ``best'' is the higher of the raw and EMA model;
``TTA'' adds horizontal-flip test-time augmentation.}
\label{tab:strong}
\begin{tabular}{c|c|c|c|c}
\hline
Model & Recipe & Epochs & Best (\%) & + TTA (\%) \\
\hline
ResNet-18      & AutoAug+Cutout+Mixup        & 300 & 96.92 & 96.96 \\
WRN-28-10      & AutoAug+Cutout              & 200 & 97.30 & 97.38 \\
WRN-28-10      & AutoAug+Cutout              & 300 & 97.43 & 97.54 \\
WRN-28-10      & AutoAug+Cutout+Mixup/CutMix & 200 & 97.56 & 97.73 \\
\textbf{WRN-28-10} & \textbf{AutoAug+Cutout+Mixup/CutMix} & \textbf{300} & \textbf{98.02} & 98.00 \\
\hline
\end{tabular}
\end{table}
...
\hline
Requirement & Evidence in this report & Main conclusion \\
\hline
VGG--A vs.\ VGG--A+BN & BN is inserted after every convolution and before
ReLU; both models are trained under the same CIFAR--10 loader and Adam setup &
BN raises best validation accuracy from \bnStdAcc\% to \bnBnAcc\% and lowers
the training-loss curve \\
Learning-rate behavior & Four learning rates are swept:
$2\times10^{-3}$, $10^{-3}$, $5\times10^{-4}$, and $10^{-4}$ &
BN is best at $10^{-3}$ and remains competitive at $5\times10^{-4}$ and
$2\times10^{-3}$, while the plain VGG--A is less stable across the sweep \\
Optimization mechanism & Loss-band and gradient-predictiveness diagnostics in
Figure~\ref{fig:bnsmooth} & BN produces a tighter loss band and a smaller
90th-percentile maximum gradient change (\bnGradBn{} vs.\ \bnGradStd{}),
supporting the smoother-landscape interpretation \\
\hline
\end{tabularx}
\end{table}
...
\hline
Angle & Evidence & Interpretation \\
\hline
Accuracy ceiling & WRN-28-10 reaches \bestAcc\% with the strongest recipe &
Large residual capacity plus modern augmentation gives the highest score. \\
Efficiency & ResNet--18 reaches \baseAcc\% with \baseParams{} parameters,
whereas WRN-28-10 uses \bestParams{} parameters & The final $2.68$-point gain
over ResNet--18 costs a much larger model and longer training schedule. \\
Generalization & No augmentation gives $100\%$ train accuracy but only
$86.46\%$ test accuracy & More training accuracy is not automatically better;
regularization and augmentation control memorization. \\
Architecture attribution & Deeper stages reach $93.08\%$, beating wider
classifier variants and the combined BN+Residual+Dropout ConvNet & Extra
convolutional depth is the strongest optional architecture change in this
controlled ConvNet setting. \\
Optimization mechanism & VGG--A+BN improves from \bnStdAcc\% to \bnBnAcc\% and
reduces p90 gradient change from \bnGradStd{} to \bnGradBn{} & BN's benefit is
mostly explained by smoother, more predictable first-order updates, not by a
large increase in parameter count. \\
Implementation credibility & Manual optimizers match PyTorch updates and the
raw-tensor ConvNet trains to \manualNetAcc\% & The report contains independent
implementation evidence rather than only black-box library calls. \\
\hline
\end{tabularx}
\end{table}
