...
# make the sibling ``utils`` package importable regardless of the working dir
...
class VGG_A_BatchNorm(nn.Module):
    """VGG_A with a BatchNorm layer inserted after every convolution.

    Identical architecture to ``VGG_A`` except that each conv is immediately
    followed by ``nn.BatchNorm2d`` (before the ReLU). This is the network used
    to study how BN affects optimisation.
    """
